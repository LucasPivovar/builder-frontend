<template>
  <div class="quiz-element" :style="wrapperStyle">
    ...
    <div v-else-if="['quiz-single','quiz-multiple','quiz-yes-no'].includes(element.type)" class="quiz-options">
      <button v-for="(option,index) in options" :key="`${index}-${option.label}`" type="button" @click.stop="toggle(index)" :class="{ selected:selected.includes(index) }">
        <span class="option-mark">{{ option.icon || (element.type === 'quiz-multiple' ? String.fromCharCode(65 + index) : '') }}</span>
...
const options=computed(()=>{
  const values = Array.isArray(props.element.options) ? props.element.options : String(props.element.optionsText||'Opção 1\nOpção 2').split('\n');
...
function normalizeOption(value){
  if(value&&typeof value==='object')return{label:String(value.label||value.text||value.title||'').trim(),description:String(value.description||value.subtitle||'').trim(),icon:String(value.icon||value.marker||'').trim()};
  return{label:String(value||'').trim(),description:'',icon:''};
}
</script>

<style scoped>
.quiz-element{width:100%;box-sizing:border-box}.quiz-progress{height:5px;border-radius:999px;background:var(--color-primary-soft);overflow:hidden}.quiz-progress span{display:block;height:100%;background:var(--color-primary);border-radius:inherit;transition:width .4s cubic-bezier(.22,1,.36,1)}.quiz-options{display:flex;flex-direction:column;gap:9px}.quiz-options button{min-height:58px;display:grid;grid-template-columns:32px 1fr 20px;align-items:center;gap:10px;width:100%;padding:11px 14px;border:1px solid var(--color-border);border-radius:14px;background:var(--color-surface);color:var(--color-text);font:inherit;font-size:15px;text-align:left;cursor:pointer;transition:transform .18s ease,border-color .18s ease,background-color .18s ease}.quiz-options button:hover,.quiz-options button.selected{border-color:var(--color-primary);background:var(--color-primary-subtle);transform:translateY(-1px)}.option-mark{width:29px;height:29px;border:1px solid var(--color-border);border-radius:50%;display:grid;place-items:center;font-size:11px;font-weight:900;transition:.18s ease}.option-copy{display:flex;min-width:0;flex-direction:column;gap:2px}.option-copy strong{font-size:15px;line-height:1.25;overflow-wrap:anywhere}.option-copy small{color:var(--color-text-muted);font-size:12px;line-height:1.25;overflow-wrap:anywhere}.quiz-options button.selected .option-mark{background:var(--color-primary);color:white;transform:scale(1.04)}.quiz-options i{color:var(--color-primary)}
